...
public class CheckMail {
    ...
    public synchronized boolean isEmailClone(String emailClone,String emailRoot){
        ...
        if(!(checkMail.checkEmail(emailClone)  && checkMail.checkEmail(emailRoot)))
            throw new IllegalArgumentException("Email is not valid");
        if(emailClone.contains("+"))throw new IllegalArgumentException("Email is not valid");
        String str="";
        String[] emailCloneSplit = emailClone.substring(0,emailClone.lastIndexOf("@")).split("[.]");
        str= StringUtils.concat(emailCloneSplit).trim();
